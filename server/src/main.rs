...
async fn connect<'r>(
    ...
    let stream = {
        ws::Stream! {
          ws => {
            for msg in chat {
              yield Message::binary(serde_json::to_vec(&msg).unwrap());
            }
            let mut ws = ws;
            loop {
              select! {
                _ = &mut shutdown => {yield Message::Close(None); break},
                recv_msg = rx.recv() => if let Ok(msg) = recv_msg {
                  yield Message::binary(msg);
                },
                sent_msg = ws.next() => if let Some(Ok(msg)) = sent_msg {
                  if !msg.is_binary() {
                    continue;
                  }
